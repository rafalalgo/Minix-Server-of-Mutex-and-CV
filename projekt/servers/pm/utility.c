...
/*===========================================================================*
 *                         tell_cv (jak w tell_vfs)                          *
 *===========================================================================*/
void tell_cv(rmp, m_ptr, signal)
struct mproc *rmp;
message *m_ptr;
int signal;
{
  for (rmp = &mproc[0]; rmp < &mproc[NR_PROCS]; ++rmp)
    if (rmp->mp_flags & IN_USE)
      if (strcmp(rmp->mp_name, "cv") == 0) {
        int CV_PROC_NR = rmp->mp_endpoint;
        
        // CS_EINTR lub CS_EXIT, w zaleznosci czy przyszlo z signal czy z forkexit
        m_ptr->m_type = signal;
        
        int r = asynsend3(CV_PROC_NR, m_ptr, AMF_NOREPLY);
        if (r != OK)
          panic("unable to send to CV: %d", r);
